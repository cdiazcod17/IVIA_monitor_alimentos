...
class Command(BaseCommand):
        ...
        while True:
            try:
                ...
                if buffer_readings:
                    ...
                    self.stderr.write(self.style.ERROR(f"Error DB: {e}"))

            except Exception as e:
                self.stderr.write(self.style.WARNING(f"Error de conexión o dispositivo no encontrado: {e}"))
    ...
    def _process_web_commands(self, device):
        ...
        # Nota: Aquí puedes iterar sobre tus dispositivos conocidos
        cmd = services.get_pending_commands(device_id=1) # Ejemplo con ID 1
        if not cmd:
            return

        if cmd['type'] == 'SET_CONFIG':
            payload = cmd['payload'] # {'power': 1, 'freq': 10, 'cant': 1}
            packet = [0]*64
            ...
            packet[5] = freq & 0xFF
            cant = payload.get('cant', 1)
            packet[6] = (cant >> 8) & 0xFF
            packet[7] = cant & 0xFF
            
            device.write(packet)
            services.mark_command_executed(cmd['id'])
            self.stdout.write(self.style.SUCCESS(f"Configuración enviada: Freq={freq}s"))